...
public class SpinningOutputThread extends Thread {
    ...
    void register(UdpSpinningChannelWriter writer) {
        for (; ; ) {
            ...
            UdpSpinningChannelWriter[] newWriters = new UdpSpinningChannelWriter[length + 1];
    ...
    void unregister(UdpSpinningChannelWriter writer) {
        for (; ; ) {
            ChannelWriters current = channelWriters;
            if (current == SHUTDOWN) {
                return;
    ...
    public void run() {
        for (; ; ) {
            ...
            for (UdpSpinningChannelWriter writer : writers.writers) {
                try {
                    writer.handle();
                } catch (Throwable t) {
                    writer.onFailure(t);
    ...
    private static class ChannelWriters {
        final UdpSpinningChannelWriter[] writers;

        ChannelWriters() {
            this(new UdpSpinningChannelWriter[0]);
        }

        ChannelWriters(UdpSpinningChannelWriter[] writers) {
            this.writers = writers;
        }

        public int indexOf(UdpSpinningChannelWriter handler) {
