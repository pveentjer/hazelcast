...
public class SpinningInputThread extends Thread {
    ...
    void register(UdpSpinningChannelReader reader) {
        for (; ; ) {
            ...
            UdpSpinningChannelReader[] newReaders = new UdpSpinningChannelReader[length + 1];
    ...
    void unregister(UdpSpinningChannelReader reader) {
        for (; ; ) {
            ChannelReaders current = channelReaders;
            if (current == SHUTDOWN) {
                return;
    ...
    public void run() {
        for (; ; ) {
            ...
            for (UdpSpinningChannelReader reader : readers.readers) {
                try {
                    reader.read();
                } catch (Throwable t) {
                    reader.onFailure(t);
    ...
    static class ChannelReaders {
        final UdpSpinningChannelReader[] readers;

        ChannelReaders() {
            this(new UdpSpinningChannelReader[0]);
        }

        ChannelReaders(UdpSpinningChannelReader[] readers) {
            this.readers = readers;
        }

        public int indexOf(UdpSpinningChannelReader reader) {
